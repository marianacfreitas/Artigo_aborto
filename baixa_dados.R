...
codigos_municipios <- read.csv(
  "databases_auxiliares/tabela_aux_municipios.csv"
) |>
  pull(codmunres) |>
  as.character()
...
source("databases_auxiliares/funcoes_auxiliares.R")
...
df_bloco2_sinasc <- df_sinasc |>
  ...
  mutate(
    ...
    total_de_nascidos_vivos_10_a_14 = if_else(
      IDADEMAE >= 10 & IDADEMAE <= 14,
      1,
      0,
      missing = 0
    ),
    total_de_nascidos_vivos_15_a_19 = if_else(
      IDADEMAE >= 15 & IDADEMAE <= 19,
      1,
      0,
      missing = 0
    ),
    total_de_nascidos_vivos_20_a_29 = if_else(
      IDADEMAE >= 20 & IDADEMAE <= 29,
      1,
      0,
      missing = 0
    ),
    total_de_nascidos_vivos_30_a_39 = if_else(
      IDADEMAE >= 30 & IDADEMAE <= 39,
      1,
      0,
      missing = 0
    ),
    total_de_nascidos_vivos_40_a_49 = if_else(
      IDADEMAE >= 40 & IDADEMAE <= 49,
      1,
      0,
      missing = 0
    ),
    total_de_nascidos_vivos_10_a_49 = if_else(
...
for (i in seq_along(arquivos)) {
  ...
  cat(paste0("Processando arquivo ", i, " de ", length(arquivos), ": ", basename(arquivo), "\n"))
  
  # Lendo o arquivo CSV
  # col_types = cols(.default = "c") força todas as colunas a serem character
  df <- read_delim(arquivo, delim = ";", escape_double = FALSE,
                   trim_ws = TRUE, col_types = cols(.default = "c"))
  
  # Extraindo a UF, ano e mês (usando regex simplificada)
...
### Exportando os dados concatenados
write.csv(
  dados_concatenados_hosp,
  "databases_auxiliares/ANS/arquivos_base_consolidada/dados_ANS_hosp_2023_2024.csv",
  row.names = FALSE
)
...
dados_concatenados_hosp_filtrados <- dados_concatenados_hosp |>
  mutate(
    ...
    cid_aborto = ifelse(
      if_any(starts_with("CID_"), ~ . %in% cids_aborto),
      1,
      0
    ),
    cid_nao_aborto = ifelse(
      if_any(starts_with("CID_"), ~ . %in% cids_nao_aborto),
      1,
      0
    )
  ) |>
...
write.csv(
  ...
  "databases_auxiliares/ANS/arquivos_base_consolidada/dados_ANS_hosp_filtrados_2023_2024.csv",
  row.names = FALSE
)
...
### Lendo e concatenando os arquivos baixados
for (ano_ref in anos_aborto) {
  cat(paste0("\n=== Iniciando Processamento do Ano: ", ano_ref, " ===\n"))
  
  # Criando o data.frame que guardará os dados do ano
  dados_concatenados_hosp_det_ano <- data.frame()
  
  for (uf_ref in estados) {
    # Criando uma lista com o nome de todos os arquivos baixados
    arquivos <- list.files(
      "databases_auxiliares/ANS/arquivos_base_detalhada",
      full.names = TRUE,
      pattern = paste0("^", uf_ref, ".*", ano_ref, ".*\\.csv$"),
      recursive = TRUE
    )
    
    # Se não encontrar arquivos para essa UF/Ano, pula para a próxima
    if (length(arquivos) == 0) next
    
    cat(paste0("  > Processando UF: ", uf_ref, " (", length(arquivos), " arquivos)\n"))
    ...
    for (i in seq_along(arquivos)) {
      ...
      # Lendo o arquivo CSV forçando tudo para character para evitar erros de bind_rows
      df <- read_delim(
        arquivo,
        delim = ";",
        escape_double = FALSE,
      ...
      # Extraindo metadados
      uf_nome  <- gsub("^.+/([A-Z]+)_\\d{6}_HOSP_DET\\.csv", "\\1", arquivo)
      ano_nome <- gsub(".+_(\\d{4})(\\d{2})_HOSP_DET\\.csv", "\\1", arquivo)
      mes_nome <- gsub(".+_(\\d{4})(\\d{2})_HOSP_DET\\.csv", "\\2", arquivo)
      
      # Adicionando colunas e convertendo tipos específicos
      df1 <- df |>
        mutate(
          Unidade_Federativa = uf_nome,
          ano = ano_nome,
          mes = mes_nome,
          # Convertendo colunas críticas (ajuste o gsub se o decimal for vírgula)
          QT_ITEM_EVENTO_INFORMADO = as.numeric(gsub(",", ".", QT_ITEM_EVENTO_INFORMADO)),
          VL_ITEM_EVENTO_INFORMADO = as.numeric(gsub(",", ".", VL_ITEM_EVENTO_INFORMADO)),
          VL_ITEM_PAGO_FORNECEDOR  = as.numeric(gsub(",", ".", VL_ITEM_PAGO_FORNECEDOR)),
      ...
      # Juntando com o restante dos dados da UF
...
for (year in anos_aborto) {
  ...
  dados_det_aborto <- dados_concatenados_hosp_det_ano |>
    filter(
      CD_PROCEDIMENTO == '31309020' |
        CD_PROCEDIMENTO == '31309062' |
        CD_PROCEDIMENTO == '31303013'
    )
  ...
  dados_aborto_det_cons_unico <- dados_aborto_det_cons |>
    ...
    select(
      ID_EVENTO_ATENCAO_SAUDE,
      CD_PROCEDIMENTO,
      Unidade_Federativa = Unidade_Federativa.x,
      ano = ano.x,
      mes = mes.x,
      SEXO,
      CD_MUNICIPIO_BENEFICIARIO,
  ...
  dados_aborto_completo <- dados_aborto_completo_aux |>
    ...
    filter(
      fet %in%
        c("abortos_ans_menor_30", "abortos_ans_30_a_39", "abortos_ans_40_a_49"),
      SEXO == "Feminino"
    ) |>
  ...
  df_numerador_aborto_ans <- bind_rows(
    df_numerador_aborto_ans,
    dados_aborto_completo_wide
  )
  
  # Limpando a memória
  rm(
    dados_aborto_completo,
    dados_aborto_completo_aux,
    dados_aborto_completo_wide,
    dados_det_aborto,
    dados_det_cons,
    dados_concatenados_hosp_det_ano,
    dados_aborto_det_cons,
    dados_aborto_det_cons_unico
  )
  gc()
}
...
df_numerador_aborto_ans[
  is.na(df_numerador_aborto_ans) & df_numerador_aborto_ans$ano %in% anos_aborto
] <- 0
...
for (ano in anos_aborto) {
  ...
  # Juntando com o restante dos dados
  df_numerador_aborto_sus <- bind_rows(
    df_numerador_aborto_sus,
    dados_filtrados_wide
  ) |>
    group_by(codmunres, ano) |>
...
df_numerador_aborto_sus <- left_join(
  df_aux_municipios,
  df_numerador_aborto_sus |>
    mutate(codmunres = as.character(codmunres), ano = as.numeric(ano))
)

### Substituindo todos os NAs, gerados após o right_join, por 0 (apenas para os anos consideradps)
df_numerador_aborto_sus[
  is.na(df_numerador_aborto_sus) & df_numerador_aborto_sus$ano %in% anos_aborto
] <- 0


## Juntando os dados das duas bases -----------------------------------------
df_numerador_aborto <- full_join(
  df_numerador_aborto_ans,
  df_numerador_aborto_sus
)
...
df_denominador_aborto <- left_join(
  ...
  mutate(
    pop_feminina_10_a_19_ans = floor((populacao_feminina_10_a_14 + populacao_feminina_15_a_19) * 0.179),
    ...
    total_de_nascidos_vivos_10_a_19_ans = floor(
      (total_de_nascidos_vivos_10_a_14 +  total_de_nascidos_vivos_15_a_19) * 0.179
    ),
    total_de_nascidos_vivos_20_a_29_ans = floor(
      total_de_nascidos_vivos_20_a_29 * 0.26
    ),
    total_de_nascidos_vivos_30_a_39_ans = floor(
      total_de_nascidos_vivos_30_a_39 * 0.32
    ),
    total_de_nascidos_vivos_40_a_49_ans = floor(
      total_de_nascidos_vivos_40_a_49 * 0.27
    ),
    pop_fem_ans_10_49 = pop_feminina_10_a_19_ans +
      pop_feminina_20_a_29_ans +
      pop_feminina_30_a_39_ans +
      pop_feminina_40_a_49_ans,
    pop_fem_sus_10_49 = populacao_feminina_10_a_49 - pop_fem_ans_10_49,
    total_de_nascidos_vivos_10_a_49_ans = total_de_nascidos_vivos_10_a_19_ans +
      total_de_nascidos_vivos_20_a_29_ans +
      total_de_nascidos_vivos_30_a_39_ans +
      total_de_nascidos_vivos_40_a_49_ans,
    total_de_nascidos_vivos_10_a_49_sus = total_de_nascidos_vivos_10_a_49 -
      total_de_nascidos_vivos_10_a_49_ans
  ) |>
  select(
    codmunres,
    ano,
    pop_fem_10_49 = populacao_feminina_10_a_49,
    pop_fem_ans_10_49,
    pop_fem_sus_10_49,
    total_de_nascidos_vivos_10_a_49,
    total_de_nascidos_vivos_10_a_49_ans,
    total_de_nascidos_vivos_10_a_49_sus
  )
...
write.csv(
  df_bloco2_novo,
  "databases/planilha_indicadores_aborto.csv",
  row.names = FALSE
)
